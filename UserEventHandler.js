class UserEventHandler {
    ...
    init(){
        ...
        this.pauseIcon.setScale(.75)
        this.pauseIcon.setInteractive({cursor:"pointer"});

        const settingWindow=this.ctx.add.rectangle(0,0,this.windowWidth,this.windowHeight,0xffffff,0.75);
        settingWindow.setOrigin(0,0);

        this.settingContainer=this.ctx.add.container(0,0);

        const restart=this.getRestartBtn(this.windowWidth/2, this.windowHeight/2-40);
        const continuation=this.getContinueBtn(this.windowWidth/2, this.windowHeight/2);
        const exit=this.getExitBtn(this.windowWidth/2, this.windowHeight/2+40);
