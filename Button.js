class Button {
  constructor(config){
    ...
    this.btnColor=config.btnColor||0xC3DC56;
    this.fontColor=config.fontColor||'#015453';
    this.fontSize=config.fontSize||20;
    this.borderColor=config.borderColor||0x008483;
    this.borderThick=config.borderThick||7;
  }
  createButtons() {
    
    const btnContainer=this.ctx.add.container(this.x,this.y);
    const btnBg=this.ctx.add.rectangle(0,0,this.width,this.height,this.btnColor);
    ...
    const mask=this.ctx.add.rectangle(0,0,this.width,this.height,0xC3DC56,0).setName("btn")
    const text = this.ctx.add.text(this.width/2, this.height/2, this.btnName, {
